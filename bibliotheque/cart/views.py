...
def reserver_tous(request):
    ...
    if request.user.is_authenticated:
        if date_debut_emprunt_tous:
            ...
            if date_debut_emprunt_tous <= today_date + timedelta(days=3) or today_date>date_debut_emprunt:
...
def reserver_exemplaire(request):
    ...
    if ouvrage.num_exmp_dispo > 0:
        utilisateur_instance = Utilisateur.objects.get(id=request.user.id)
...
def cart_summary(request):
    ...
    if is_staff_no_auth(request):
        return render_admin(request,cart_exemplaires)
    else:
        return render_user(request,cart_exemplaires)


@login_required
def render_admin(request,cart_exemplaires):
